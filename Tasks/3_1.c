...
// Szekvenciális összegző függvény
long sequential_sum(int *arr, long n) {
    long sum = 0;
    for (long i = 0; i < n; i++) {
        sum += arr[i];
    }
...
double measure_time(void (*func)(int*, long, int), int *arr, long n, int num_threads) {
    clock_t start, end;
    start = clock();
    long result = func(arr, n, num_threads);
    end = clock();
...
int main() {
    ...
    double seq_time = measure_time((void (*)(int*, long, int))sequential_sum, arr, n, 1);
